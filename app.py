...
def run_ml_app():
    ...
    self_employee = left.selectbox("Self Employed", ("Yes", "No"))
    applicant_income = right.number_input("Applicant Income")
    coAppliant_income = left.number_input("coApplicant Income")
...
def predict(gender, married, dependent, education, self_employed, applicant_income, coApplicant_income
    ...
    pro = 0 if property_area == "Semiurban" else if property_area == "Urban" else 2
                             
    #Making prediction
    prediction = Logistic.Regression_Model:predict(
